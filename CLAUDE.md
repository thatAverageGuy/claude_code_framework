...
This file covers core engineering processes. GTM, sales, outreach,
and memory capabilities live in .claude/agents/ and .claude/skills/
and load on demand. See EXTENSION REGISTRY below for what's available.
...
│
├── .claude/                         # Claude Code extensions (project-level)
...
│   │   ├── VISION.md               # Purpose, audience, anti-goals (Phase 0)
│   │   ├── ARCHITECTURE.md          # Living architecture overview
│   │   ├── SPEC.md                  # Technical specification & requirements
│   │   ├── DEPLOY.md               # Deployment config, runbook, rollback
...
│   │   │   ├── ADR-001.md           # Tech choices are ADRs too
│   │   │   └── ...
│   │   │
...
│   │
│   ├── client/                      # Client-facing (PROJECT_TYPE: client only)
│   │   ├── PROPOSAL_DRAFT.md
│   │   ├── SOW_DRAFT.md
│   │   └── HANDOFF.md
│   │
│   └── gtm/                         # Go-to-market (when PROJECT_INTENT set)
...
PROJECT_INTENT:     [not-set | learning | build-in-public |
                     commercial-saas | consulting-showcase |
                     community-tool | client-delivery]
CURRENT_PHASE:      [0-vision | 1-requirements | 2-tech-design |
                     3-bootstrap | 4-implementation | 5-validation |
                     6-deployment | N/A (existing project)]
```

### PROJECT_INTENT Behavior
- PROJECT_INTENT is OPTIONAL. Default: `not-set`
- When `not-set`: all GTM/sales skills remain available but are not
  auto-suggested. No additional deliverables are generated.
- User can set/change intent at any time: "set intent to build-in-public"
- When set, phase deliverables adjust per the INTENT MODIFIER TABLE
  in the Greenfield Lifecycle section.
- Intent affects task planning: build-in-public adds content tasks,
  commercial-saas adds billing/auth tasks, etc.

────────────────────────────────────────────────────────
## PERSISTENT MEMORY
────────────────────────────────────────────────────────
**Structured memory system** lives at `~/.claude/memory/`.
See `~/.claude/memory/MEMORY_SCHEMA.md` for architecture.

This section is for QUICK session-relevant overrides only.
Long-term facts go in `~/.claude/memory/semantic/`.
Learned patterns go in `~/.claude/memory/procedural/`.

When the user says "permanently store this", "remember this permanently",
or similar:
1. Determine memory type (semantic fact, procedural pattern, or goal)
2. Update the appropriate file in `~/.claude/memory/`
3. If memory system is not yet set up, fall back to this section

### Pruning Rules
- Review this section at the start of each session
- Migrate entries to ~/.claude/memory/ when the system is active
- Remove entries that are no longer relevant
- If this section exceeds 15 lines, flag it to user for cleanup
...
## EXTENSION REGISTRY
────────────────────────────────────────────────────────
Sub-agents and skills load on demand — they do NOT consume
context unless invoked. This table is the quick-reference.

### Sub-Agents (~/.claude/agents/ or .claude/agents/)

| Agent              | Purpose                                   | Model   | Phase  |
|--------------------|-------------------------------------------|---------|--------|
| researcher         | Domain research, tech eval, competitor    | sonnet  | 1, 2   |
|                    | analysis. Spawn multiple for parallel     |         |        |
|                    | comparison. Override to inherit for deep  |         |        |
|                    | technical eval.                           |         |        |
| doc-generator      | Bulk doc creation from structured input.  | sonnet  | 3      |
|                    | Writes files directly.                    |         |        |
| quality-gate       | Code review + test audit combined.        | inherit | 5      |
|                    | Reads many files, returns findings.       |         |        |
| deploy-validator   | Deployment readiness: CI/CD, containers,  | sonnet  | 6      |
|                    | security, config, monitoring.             |         |        |
| gtm-strategist     | Market positioning, pricing, competitive  | sonnet  | GTM    |
|                    | landscape, launch planning.               |         |        |
| outreach-specialist| Cold emails, prospect research, outreach  | sonnet  | Sales  |
|                    | campaigns, demo project scaffolding.      |         |        |

### Skills (~/.claude/skills/ or .claude/skills/)

| Skill              | Invocation   | Context | Purpose                     |
|--------------------|--------------|---------|-----------------------------|
| /new-project       | User-only    | Main    | Phase 0 kickoff interview   |
| /tech-eval         | User-only    | Fork    | Parallel tech comparison    |
| /phase-gate        | User-only    | Main    | Validate phase exit criteria|
| /bootstrap         | User-only    | Main    | Scaffold project structure  |
| /deploy-readiness  | User-only    | Fork    | Full deployment checklist   |
| /commercialize     | User-only    | Fork    | Commercial strategy creation|
| /promote           | User-only    | Fork    | Visibility/community plan   |
| /prospect          | User-only    | Fork    | Outreach campaign planning  |

All skills are disable-model-invocation: true (user-triggered only,
they have side effects or represent workflow transitions).

### Context Strategy Principle
Use sub-agents when the task:
- Reads many files (exploration pollutes main context)
- Produces verbose output (you want the summary, not the journey)
- Can run in parallel (multiple researchers evaluating different options)
- Is self-contained (doesn't need back-and-forth with user)

Keep in main thread when the task:
- Requires user decisions or judgment calls
- Is lightweight/conversational (interviews, Q&A)
- Needs access to full conversation history
...
────────────────────────────────────────────────────────
Phase-gated pipeline for new projects from scratch.
Enter at any phase (if earlier phases are satisfied).
Gates are ADVISORY: Claude checks and reports, user decides.

```
P0 VISION ──► P1 REQUIREMENTS ──► P2 TECH DESIGN ──►
P3 BOOTSTRAP ──► P4 IMPLEMENTATION ──► P5 VALIDATION ──►
P6 DEPLOYMENT
```
...
```
Entry:     User invokes /new-project OR declares "new project"
Thread:    MAIN (lightweight, conversational)
Level:     0 (read only — no file changes yet)
```

**Activities:**
1. Ask PROJECT_TYPE explicitly (personal | client | open-source)
2. Ask PROJECT_INTENT (offer all options, accept "not decided yet")
3. Structured interview — adapt depth to starting clarity:
   - Problem statement: what pain does this solve?
   - Target users: who cares? (reject "everyone")
   ...
   - For client projects: client context, stakeholders,
     contractual constraints, billing model
   - For build-in-public: target audience, platforms,
     content angle, sharing cadence
4. Synthesize into VISION.md draft
5. User approval

**Deliverables:**
- docs/dev/VISION.md (approved)
- PROJECT_TYPE and PROJECT_INTENT set in CLAUDE.md
- For client: docs/client/PROPOSAL_DRAFT.md (if applicable)

**Exit Criteria:**
- [ ] Problem statement is specific and falsifiable
- [ ] Target users identified (not "everyone")
- [ ] At least 3 anti-goals documented
- [ ] Success criteria are measurable
- [ ] User has approved VISION.md

### Phase 1: REQUIREMENTS

```
Entry:     Phase 0 exit criteria satisfied (or user declares satisfied)
Thread:    MAIN (decisions) + researcher sub-agent for domain research
Level:     0 (read only)
```

**Activities:**
1. If domain research needed (client industry, competitor landscape,
   existing solutions), spawn `researcher` sub-agent to gather info.
   Main thread stays clean.
2. Elicit functional requirements through structured conversation:
...
4. For commercial-saas intent: monetization model, pricing tiers,
   multi-tenancy requirements
5. For client projects: generate SOW draft
6. Prioritize: MoSCoW (Must/Should/Could/Won't) for v1
7. Synthesize into SPEC.md

**Deliverables:**
- docs/dev/SPEC.md (requirements and constraints)
- For client: docs/client/SOW_DRAFT.md
- For commercial-saas: pricing model section in SPEC.md

**Exit Criteria:**
- [ ] All must-have features have clear acceptance criteria
...
```
Entry:     Phase 1 exit criteria satisfied
Thread:    MAIN (decisions) + researcher sub-agents (parallel eval)
Level:     0 (read only)
```

**Activities:**
1. Identify technology decisions needed:
   - Language/runtime
   - Framework(s)
   - Database(s)
   - Infrastructure/hosting
   - Key libraries (auth, ORM, queue, etc.)
2. For each decision, invoke /tech-eval OR spawn parallel
   `researcher` sub-agents to evaluate options. Each agent
   evaluates ONE option and returns:
   - Fit for requirements (from SPEC.md)
   - Maturity, community, maintenance status
   - Performance characteristics
   - Learning curve / team familiarity
   - Lock-in risk
   - Cost implications
3. Present comparison table to user for each decision
4. Create ADR for each technology choice
5. Design high-level architecture:
   - Component diagram
   - Data flow
   - State machines (project-level + per component)
   - Deployment topology
6. For build-in-public: identify "explainable choices" worth posting about
7. Write ARCHITECTURE.md draft

**Deliverables:**
- docs/dev/adr/ADR-001.md through ADR-N (one per tech decision)
- docs/dev/ARCHITECTURE.md (draft)
- docs/dev/STATE_MACHINES.md (draft)

**Exit Criteria:**
- [ ] All technology choices have ADRs with alternatives documented
- [ ] Architecture diagram shows all major components
- [ ] Data flow is documented
- [ ] State machines cover project-level and all stateful components
- [ ] No unresolved "TBD" items in architecture
- [ ] User has approved all ADRs and architecture

### Phase 3: PROJECT BOOTSTRAP

```
Entry:     Phase 2 exit criteria satisfied
Thread:    MAIN (approval) + doc-generator sub-agent for bulk docs
Level:     2 (local — creating files)
```

**Activities:**
1. Scaffold project directory structure:
   - Source directories per architecture
   - Test directories mirroring source
   - docs/ structure from template
   - CI/CD placeholder
   - Dockerfile placeholder
   - Config files (linter, formatter, package manager)
2. Initialize git with dev branch
3. Spawn `doc-generator` sub-agent with all Phase 0-2 decisions
   to generate remaining docs:
   - CONTRIBUTING.md (conventions from ADRs)
   - CONFIG_REFERENCE.md (from architecture)
   - docs/README.md (index)
4. Create initial TASKS.md with v1 feature backlog from SPEC.md
5. Create DETAIL.md for first task
6. Write CONTEXT.md (initial state)
7. Populate PROJECT-SPECIFIC CONTEXT in CLAUDE.md
8. For commercial-saas: scaffold auth, billing, multi-tenancy stubs
9. For build-in-public: create docs/gtm/CONTENT_PLAN.md,
   CONTENT_CALENDAR.md linking milestones to content
10. Present ALL generated files to user for approval

**Deliverables:**
- Full project scaffold with docs
- TASKS.md with prioritized backlog
- CONTEXT.md initialized
- CLAUDE.md project section populated
- For applicable intents: docs/gtm/ files

**Exit Criteria:**
- [ ] Project builds/runs (even if empty)
- [ ] All docs generated and approved
- [ ] TASKS.md has at least first sprint of tasks
- [ ] First task has DETAIL.md ready for implementation
- [ ] Git initialized, initial commit on dev

### Phase 4: IMPLEMENTATION

```
Entry:     Phase 3 exit criteria satisfied
Thread:    MAIN (existing task system from TASK CONVENTIONS section)
Level:     Per task (1, 2, or 3 as declared)
```

This phase uses the existing TASK CONVENTIONS, COMMIT PROCEDURE,
and CHANGE LEVEL systems already defined in this CLAUDE.md.

**Additional activities based on PROJECT_INTENT:**
- build-in-public: after completing milestone tasks, prompt user
  to create a content piece (draft with /promote or manually)
- commercial-saas: include billing/auth/onboarding tasks in backlog
- client-delivery: track against SOW deliverables

### Phase 5: VALIDATION & HARDENING

```
Entry:     All v1 must-have tasks DONE
Thread:    MAIN (review) + quality-gate sub-agent
Level:     1-2 (fixes only)
```

**Activities:**
1. Spawn `quality-gate` sub-agent to review entire codebase:
   - Code quality against conventions
   - Test coverage and test quality
   - Error handling completeness
   - Security review (input validation, auth, secrets)
   - Performance review (N+1 queries, memory leaks, etc.)
2. Address findings (create BF- tasks for issues)
3. User acceptance testing
4. For client: walkthrough with client stakeholders
5. For commercial-saas: pricing/onboarding UX review
6. For build-in-public: public documentation review

**Deliverables:**
- Quality gate findings report (from sub-agent)
- All critical/blocking issues resolved
- docs/user/TROUBLESHOOTING.md (updated)
- For client: docs/client/HANDOFF.md (draft)

**Exit Criteria:**
- [ ] quality-gate reports no critical issues
- [ ] Test suite passes with documented coverage
- [ ] User acceptance criteria from SPEC.md all satisfied
- [ ] No unresolved security findings

### Phase 6: DEPLOYMENT & HANDOFF

```
Entry:     Phase 5 exit criteria satisfied
Thread:    MAIN (approval) + deploy-validator sub-agent
Level:     2 (deployment config changes)
```

**Activities:**
1. Spawn `deploy-validator` sub-agent to run full checklist
2. Address findings
3. Generate DEPLOY.md
4. Execute deployment to target environment
5. Smoke test in production
6. For client: finalize HANDOFF.md, conduct handoff session
7. For build-in-public: execute launch plan
8. For commercial-saas: landing page, launch sequence

**Deployment Checklist (run by deploy-validator):**

CI/CD:
- [ ] Pipeline defined and all stages passing
- [ ] Stages present: lint → test → build → deploy
...
- [ ] Multi-stage build (no dev deps in prod image)
- [ ] Image size < [project-specific threshold]
- [ ] Health check endpoint defined in container
- [ ] Non-root user configured
- [ ] .dockerignore excludes unnecessary files

Configuration:
- [ ] Environment-specific configs separated (dev/staging/prod)
- [ ] All env vars documented in CONFIG_REFERENCE.md
- [ ] Sensitive values use secrets manager (not env files)
...
- [ ] Health endpoint returns service status + dependency status
- [ ] Error tracking configured (Sentry, etc.)
- [ ] Key business metrics identified and instrumented
...
- [ ] DEPLOY.md has runbook for common operations
- [ ] Rollback procedure documented and tested
- [ ] On-call/escalation path documented (if applicable)

**Deliverables:**
- docs/dev/DEPLOY.md (approved)
- docs/user/TROUBLESHOOTING.md (final)
- Successful deployment + smoke test
- For client: docs/client/HANDOFF.md (final)
- For build-in-public: launch posts drafted/published

**Exit Criteria:**
- [ ] All checklist items green or explicitly waived with reason
- [ ] DEPLOY.md approved by user
- [ ] Successful deployment to target environment
- [ ] Smoke test passing in deployed environment
- [ ] For client: handoff session completed

### INTENT MODIFIER TABLE

When PROJECT_INTENT is set, phases produce additional deliverables:

| Phase | build-in-public        | commercial-saas         | consulting-showcase    | client-delivery        |
|-------|------------------------|-------------------------|------------------------|------------------------|
| P0    | Audience, platforms    | Market, pricing model   | Portfolio positioning  | Proposal draft         |
| P1    | Content angle/feature  | Monetization model      | Demo scope definition  | SOW draft              |
| P2    | Explainable choices    | Scalability, tenancy    | Impressive tech picks  | Client constraints     |
| P3    | Content calendar       | Billing/auth scaffold   | Demo-ready scaffold    | Client env setup       |
| P4    | Content per milestone  | SaaS infra tasks        | Showcase milestones    | SOW deliverable track  |
| P5    | Public doc review      | Pricing/onboarding UX   | Demo walkthrough       | Client UAT             |
| P6    | Launch plan, posts     | Landing page, launch    | Case study draft       | Handoff + HANDOFF.md   |

────────────────────────────────────────────────────────
## EXISTING PROJECT ONBOARDING
────────────────────────────────────────────────────────
Use this protocol when entering an EXISTING project for the first time
OR when re-syncing after state drift. For NEW projects from scratch,
use the GREENFIELD PROJECT LIFECYCLE above.
